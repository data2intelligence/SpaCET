...
SpaCET is an R package for analyzing cancer spatial transcriptomics (ST) datasets to estimate cell lineages and intercellular interactions in the tumor microenvironment. Briefly, SpaCET first infers cancer cell abundance by integrating a gene pattern dictionary of common malignancies. SpaCET then uses a constrained regression model to calibrate local tissue densities and determine stromal and immune cell lineage fraction. Further, SpaCET can reveal putative cell-cell interactions in tumor microenvironment. Of note, although SpaCET does not require any input cell references to analyze tumor ST data, SpaCET can still accept a matched scRNA-seq dataset as customized references to conduct cell type deconvolution of ST data.
...
* R version >= 4.2.0. 
* R packages: Matrix, jsonlite, ggplot2, reshape2, patchwork, png, shiny, plotly, DT, MUDAN, factoextra, NbClust, cluster, parallel, psych, BiRewire, limma.
...
## Citation
Beibei Ru, Jinlin Huang, Yu Zhang, Kenneth Aldape, Peng Jiang. Estimation of cell lineages in tumors from spatial transcriptomics data. Nature Communications 14, 568 (2023). [<a href="https://www.nature.com/articles/s41467-023-36062-6" target="_blank">Link</a>]
