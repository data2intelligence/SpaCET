...
To read your ST data into R, user can create an SpaCET object by using `create.SpaCET.object` or `create.SpaCET.object.10X`. Specifically, if users are analyzing a ST dataset from 10x Visium, they only need to input "visiumPath" by using `create.SpaCET.object.10X`. Please make sure that "visiumPath" points to the standard output folders of 10x SpaCET Ranger, which has both "filtered_feature_bc_matrix" and "spatial" folders.

Here, we create an SpaCET object for the current breast cancer visium dataset. In this step, the genes exhibiting zero UMI counts across spots will be removed. Also, if there are duplicated gene symbols, the gene with the highest total counts remains and all other duplicated genes are removed.
...
``` r
# show this object.
SpaCET_obj@input$counts[1:8,1:3]
...
We provide `SpaCET.visualize.deconvolution` to present the spatial distribution of cell types by setting the parameter `cellType`.

``` r
# show the spatial distribution of malignant cells and macrophages.
p1 <- SpaCET.visualize.deconvolution(SpaCET_obj, cellType="Malignant")
p2 <- SpaCET.visualize.deconvolution(SpaCET_obj, cellType="Macrophage")
p1 | p2
```
<img src="img/visualizeDeconvolution.png" width="100%" style="border:none" />

By setting the parameter `interactive` as `TRUE`, user will open up a Shiny plot to interactively visualize each cell type. Of note, each dot represents an ST spot and user can adjust the size and opacity of spots.

``` r
# show the spatial distribution of malignant cells and macrophages.
SpaCET.visualize.deconvolution(SpaCET_obj, cellType="Malignant", interactive=TRUE)
```
...
SpaCET_obj@results$deconvolution[c("Cancer cell state A","Cancer cell state B"),1:3]

##                     AAACAAGTATCTCCCA-1 AAACACCAATAACTGC-1 AAACAGAGCGACTCCT-1
## Cancer cell state A         0.21735562       9.999900e-01       6.845963e-02
## Cancer cell state B         0.06870794       1.239573e-11       2.906452e-08

SpaCET.visualize.deconvolution(SpaCET_obj, cellTypes=c("Malignant","Cancer cell state A","Cancer cell state B"),ncol=3)
``` 
