...
To read your ST data into R, user can create an SpaCE object by using `create.SpaCE.object` or `create.SpaCE.object.10X`. Specifically, if users are analyzing a ST dataset from 10x Visium, they only need to input "visiumPath" by using `create.SpaCE.object.10X`. Please make sure that "visiumPath" points to the standard output folders of 10x Space Ranger, which has both "filtered_feature_bc_matrix" and "spatial" folders.

Here, we create an SpaCE object for the current breast cancer visium dataset. In this step, the genes exhibiting zero UMI counts across spots will be removed. Also, if there are duplicated gene symbols, the gene with the highest total counts remains and all other duplicated genes are removed.
...
The UMI count matrix is stored in `SpaCE_obj@input$counts` as a sparse matrix. Each row represents a gene whereas each column is a spot.

``` r
# show this object.
SpaCE_obj@input$counts[1:8,1:3]
...
SpaCE_obj@results$deconvolution[c("Cancer cell state A","Cancer cell state B"),1:3]

##                     AAACAAGTATCTCCCA-1 AAACACCAATAACTGC-1 AAACAGAGCGACTCCT-1
## Cancer cell state A         0.21735562       9.999900e-01       6.845963e-02
## Cancer cell state B         0.06870794       1.239573e-11       2.906452e-08

SpaCE.visualize.deconvolution(SpaCE_obj, cellTypes=c("Malignant","Cancer cell state A","Cancer cell state B"),ncol=3)
``` 
