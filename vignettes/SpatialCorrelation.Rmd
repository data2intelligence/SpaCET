...
In this vignette, we demonstrate the use of `SpaCET` to identify (1) spatially variable genes, (2) spatially co-expressed ligand–receptor interactions, and (3) pairwise co-expressed gene pairs, all of which are primarily based on Moran’s I measures. According to a benchmarking [study](https://doi.org/10.1186/s13059-025-03731-2){target="_blank"}, Moran’s I achieves competitive performance in detecting spatial patterns.
...
The spatial weight matrix W is calculated based on the Radial Basis Function (RBF) kernel as follows. dij denotes the Euclidean distance between spot i and j on spatial coordinates in the um space. We assigned the free parameter sigma = 100 because the distance between a spot and its 1st layer of neighbor spots is 100 um. We set Wii = 0 to mask coexpression effects in the same spot. We also set Wij = 0 when the distance between spots i and j is over 200 um, i.e., beyond two layers of neighbor spots in Visium data.

<center><img src="img/w.png" width="25%" style="border:none" /></center>
...
<center><img src="img/m1.png" width="60%" style="border:none" /></center>
...
# run spatial correlation, 2 mins
...
# show results
SpaCET_obj@results$SpatialCorrelation$univariate[1:6,]

```

## Spatially co-expression ligand-receptor interactions
...
<center><img src="img/m2.png" width="60%" style="border:none" /></center>
...
# show results
SpaCET_obj@results$SpatialCorrelation$bivariate[1:6,]
